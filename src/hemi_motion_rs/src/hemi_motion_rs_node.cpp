...
class HemiMotionRSNode : public rclcpp::Node
{
public:
  HemiMotionRSNode()
  : Node("hemi_motion_rs_node")
  {
    // --- Parameters ---
    ...
    // NEW: structured data layout
    base_data_dir_ = declare_parameter<std::string>(
      "base_data_dir", "/root/ur_ws_sim/data");
    run_id_ = declare_parameter<std::string>(
      "run_id", "run_001");
    camera_name_ = declare_parameter<std::string>(
      "camera_name", "realsense");

    // e.g. /root/ur_ws_sim/data/run_100
    run_dir_ = base_data_dir_ + "/" + run_id_;
    // e.g. /root/ur_ws_sim/data/run_100/images/realsense
    save_dir_ = run_dir_ + "/images/" + camera_name_;
    // e.g. /root/ur_ws_sim/data/run_100/calib
    calib_dir_ = run_dir_ + "/calib";

    RCLCPP_INFO(get_logger(),
                "Hemi RS: group='%s', ik_frame='%s', target_frame='%s', pipeline='%s', planner='%s'",
    ...
    RCLCPP_INFO(get_logger(), "Base data dir: %s", base_data_dir_.c_str());
    RCLCPP_INFO(get_logger(), "Run id       : %s", run_id_.c_str());
    RCLCPP_INFO(get_logger(), "Camera name  : %s", camera_name_.c_str());
    RCLCPP_INFO(get_logger(), "Run dir      : %s", run_dir_.c_str());
    RCLCPP_INFO(get_logger(), "Save dir     : %s", save_dir_.c_str());

    // Make sure directories exist
    {
      std::string cmd_images = "mkdir -p " + save_dir_;
      std::system(cmd_images.c_str());
      
      std::string cmd_calib = "mkdir -p " + calib_dir_;
      std::system(cmd_calib.c_str());
    }

    // NEW: open robot pose log (per run)
    {
      std::string pose_log_path = calib_dir_ + "/robot_poses.csv";
      pose_log_.open(pose_log_path, std::ios::out | std::ios::trunc);
      if (!pose_log_) {
        RCLCPP_WARN(get_logger(), "Failed to open pose log file: %s",
                    pose_log_path.c_str());
      } else {
        // CSV header
        pose_log_ << "image_index,image_name,frame_id,"
                  << "px,py,pz,"
                  << "qx,qy,qz,qw\n";
        RCLCPP_INFO(get_logger(), "Logging robot poses to: %s",
                    pose_log_path.c_str());
      }
    }
    ...
    image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
      image_topic_, 10,
      std::bind(&HemiMotionRSNode::imageCallback, this, std::placeholders::_1));
  ...
  // Must be called after make_shared()
...
private:
  ...
  // returns image name (basename) or empty string on failure
  std::string saveCurrentImage(size_t pose_index)
  {
    std::lock_guard<std::mutex> lock(image_mutex_);

    if (!last_image_) {
      RCLCPP_WARN(this->get_logger(),
                  "No image received yet, cannot save for pose %zu.", pose_index);
      return "";
    }
    ...
    name_ss << "img_" << std::setw(4) << std::setfill('0') << image_counter_++
            << "_pose_" << std::setw(4) << std::setfill('0') << pose_index << ".png";
    std::string image_name = name_ss.str();

    // full path on disk
    std::string full_path = save_dir_ + "/" + image_name;

    if (!cv::imwrite(full_path, cv_ptr->image)) {
      RCLCPP_ERROR(this->get_logger(), "Failed to write image to %s", full_path.c_str());
      return "";
    }

    RCLCPP_INFO(this->get_logger(), "Saved image %s", full_path.c_str());
    return image_name;  // important: we return the name for pose logging
  }

  void logRobotPose(size_t pose_index, const std::string &image_name)
  {
    if (!pose_log_.is_open()) {
      return;
    }

    // Get current pose of the end-effector in whatever frame MoveIt uses
    geometry_msgs::msg::PoseStamped ee_pose =
      move_group_->getCurrentPose(ik_frame_);

    const auto &p = ee_pose.pose.position;
    const auto &q = ee_pose.pose.orientation;
    ...
    // Optional: flush to be safe if something crashes mid-run
    pose_log_.flush();
  }

  // ===== pose generation & visualization (same as your original) =====
  void generatePoses()
  {
    poses_.clear();
  ...
  void runMotion()
  {
    ...
    for (size_t i = 0; i < poses_.size() && rclcpp::ok(); ++i) {
      ...
      // === Capture image + log robot pose ===
      std::string image_name = saveCurrentImage(i);
      if (!image_name.empty()) {
        logRobotPose(i, image_name);
      }
  ...
  // NEW: run / camera layout
  std::string base_data_dir_;
  std::string run_id_;
  std::string camera_name_;
  std::string run_dir_;   // e.g. /root/ur_ws_sim/data/run_100
  std::string save_dir_;  // e.g. /root/ur_ws_sim/data/run_100/images/realsense
  std::string calib_dir_;  // e.g. /root/ur_ws_sim/data/run_100/calib
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<HemiMotionRSNode>();
