...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Base directory where run folders and calib/ live. "
             "Default: /root/ur_ws_sim/data",
    )
    ...
    parser.add_argument(
        ...
        help="Optional explicit calibration file path (.npz). "
             "If not given, uses <base-dir>/calib/CameraParams_<camera>.npz",
    )
    ...
    parser.add_argument(
        ...
        help="Optional explicit output path for poses (.npz). "
             "If not given, saves to <base-dir>/<run-id>/poses_<camera>.npz",
    )
...
def load_calibration(base_dir, camera_name, calib_file=None):
    """
...
def find_chessboard_corners(image_dir, nx, ny, show=False):
    ...
    all_results = []  # list of (image_path, gray, corners_refined)
...
def save_poses(base_dir, run_id, camera_name, output_path,
    ...
    Save results to npz:
      ...
      - rvecs:     (N, 3)
      - tvecs:     (N, 3)
      - K, dist:   intrinsics (copied for convenience)
    """
    if output_path is None:
        run_dir = os.path.join(base_dir, run_id)
        os.makedirs(run_dir, exist_ok=True)
        output_path = os.path.join(run_dir, f"poses_{camera_name}.npz")

    # Pack into clean arrays
    ...
    print(f"\n📁 Saved poses to: {output_path}")
    print("Each pose is board-in-camera: T_board^camera (rvec, tvec).")
...
def main():
    ...
    # 1) Load intrinsics
    K, dist = load_calibration(args.base_dir, args.camera_name, args.calib_file)
